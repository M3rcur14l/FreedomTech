		...
		<div class="large-6 small-9 columns ">
			...
			<div class="large-6 columns">
				<ul class="inline-list">
					<li><a href="<c:url value="/friendslist"/>">Friends</a></li>
					<li><a href="<c:url value="/friendsGraph"/>">Graph</a></li>
					<li class="right"><a href="<c:url value="/signout"/>">Logout</a></li>
				</ul>

				<ul>
					<c:forEach items="${selectedFriends}" var="friend">
						<li>${facebook.getUserProfile(friend).getName()}<ul>
								<c:forEach items="${friendsMap[friend]}" var="commonFriend">
									<li>${facebook.getUserProfile(commonFriend).getName()}</li>
								</c:forEach>
							</ul></li>
					</c:forEach>
