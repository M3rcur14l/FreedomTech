	...
	<p id="breadcrumbs">
		...
		&gt; <a
			href="<c:url value="/type-view?category=${pageCategory.name}"/>"><c:out
				value="${pageCategory.name}" /></a> &gt;
		<c:out value="${pageType.name}" />
	</p>
	<article>
		<h1>
			<c:out value="${pageType.name}" />
		...
		<p>
			<c:out value="${pageType.description}" />
		</p>
	</article>
	<ul class="card-list">
		<c:forEach items="${productList}" var="product">
			<li><a
				href="<c:url value="/product-detail-view?category=${product.type.category.name}
										&type=${product.type.name}&product=${product.name}"/>">
					<img src="<c:url value="/resources/img/${product.name}.png"/>">
